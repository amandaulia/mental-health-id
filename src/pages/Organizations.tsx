  ...
  const filteredData = useMemo(() => {
    ...
    const filtered = (dbOrganizations as any[]).filter((item) => {
      ...
      if (filters.search) {
        ...
        if (!item.name.toLowerCase().includes(searchLower) && !city.toLowerCase().includes(searchLower)) return false;
      }
    ...
    <div className="container mx-auto px-4 py-8 sm:py-12">
      ...
      <div className="mb-8 sm:mb-12 text-center">
        <h1 className="text-3xl sm:text-5xl font-bold mb-4 sm:mb-6">
          <span className="gradient-text">{t("pages.organizations.heroTitleA")}</span>
          {t("pages.organizations.heroTitleB")}
        </h1>
      ...
      <div>
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl sm:text-2xl font-semibold text-foreground">{t("organizations.all")}</h2>
        </div>
